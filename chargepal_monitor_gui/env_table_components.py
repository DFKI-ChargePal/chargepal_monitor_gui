...
def delete_row(e: events.GenericEventArguments, table: ui.table, row_name: str) -> None:
    global table_rows
    table_rows[row_name][:] = [
        r for r in table_rows[row_name] if r['id'] != e.args['id']
    ]
    table.update()
...
def create_tables():
    ...
    cart_table = ui.table(
        ...
        rows=table_rows['cart_rows'],
        row_key='cart_names',
        title='Cart details',
    ).classes('w-80')
    ads_table = ui.table(
        columns=ads_columns,
        rows=table_rows['ads_rows'],
        row_key='ads',
        title='ADS Station details',
    ).classes('w-80')
    bcs_table = ui.table(
        columns=bcs_columns,
        rows=table_rows['bcs_rows'],
        row_key='bcs',
        title='BCS Station details',
    ).classes('w-80')
...
def add_env_table_slots(tables: ui.table):
    ...
    tables['ads_table'].on(
        'rename', lambda e: rename(e, tables['ads_table'], 'ads_rows')
    )
    tables['ads_table'].on(
    ...
    tables['bcs_table'].on(
        'rename', lambda e: rename(e, tables['bcs_table'], 'bcs_rows')
    )
    tables['bcs_table'].on(
    ...
    tables['cart_table'].on(
        'rename', lambda e: rename(e, tables['cart_table'], 'cart_rows')
    )
    tables['cart_table'].on(
