...
import socket
import json
from chargepal_monitor_gui.srv import readRobotInfo,readRobotInfoResponse,updateRobotInfo,updateRobotInfoResponse
from chargepal_monitor_gui.srv import readCartInfo,readCartInfoResponse,updateCartInfo,updateCartInfoResponse
...
def addRobot():
    global robotCounter
    robot_info = [f"Chargepal{robotCounter}","unknown","unknown","unknown","unknown","unknown","unknown","unknown"]
...
def setCartInfo(req):
    ...
    for row in all_rows:
        ...
        if cart_data['cart_name'] == original_values[0]:
            ...
            cartData[original_values[0]] = dict(zip(cart_columns[1:],updated_values[1:]))
...
if __name__ == "__main__":
    ...
    get_robotinfo = rospy.Service('chargepal_services/get_robot_info', readRobotInfo,getRobotData)
    set_robotinfo = rospy.Service('chargepal_services/set_robot_info', updateRobotInfo, setRobotInfo)
    get_cartinfo = rospy.Service('chargepal_services/get_cart_info', readCartInfo,getCartData)
    set_cartinfo = rospy.Service('chargepal_services/set_cart_info', updateCartInfo, setCartInfo)
    ...
    addRobotButton.pack()
    delRobotButton.pack()
    robotTable = ttk.Treeview(robotSection, columns=("robot_name", "current_job", "ongoing_action", "previous_action", "robot_position", "cart_present", "charging", "battery"))
    ...
    addCartButton.pack()
    delCartButton.pack()
    cartTable = ttk.Treeview(cartSection, columns=("cart_name", "cart_position", "plug_connected", "robot_with_cart"))
